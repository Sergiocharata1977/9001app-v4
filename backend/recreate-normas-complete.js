...
async function recreateNormasTable() {
  try {
    console.log('🔧 Recreando tabla normas completa...');

    // 1. Eliminar tabla existente
    console.log('🗑️ Eliminando tabla normas existente...');
    await tursoClient.execute({
      sql: 'DROP TABLE IF EXISTS normas',
      args: []
    });

    // 2. Crear tabla completa
    console.log('🏗️ Creando nueva tabla normas...');
    await tursoClient.execute({
      sql: `CREATE TABLE normas (
        ...
        descripcion TEXT,
        version TEXT DEFAULT '2015',
        tipo TEXT DEFAULT 'ISO 9001',
    ...
    console.log('✅ Tabla normas creada con estructura completa');

    // 3. Insertar puntos de ISO 9001:2015
    console.log('📝 Insertando puntos de ISO 9001:2015...');
    
    for (const punto of puntosISO9001) {
      await tursoClient.execute({
        sql: `INSERT INTO normas (codigo, titulo, descripcion, observaciones, organization_id, version, tipo, estado) 
              VALUES (?, ?, ?, ?, 21, '2015', 'ISO 9001', 'activo')`,
    ...
    const examples = await tursoClient.execute({
      sql: 'SELECT codigo, titulo FROM normas WHERE organization_id = 21 LIMIT 5',
      args: []
    });

    console.log('📋 Ejemplos de normas insertadas:');
    examples.rows.forEach(row => {
      console.log(`  - ${row.codigo}: ${row.titulo}`);
    });
