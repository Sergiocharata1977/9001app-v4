...
const logAuditAction = async (
  ...
  try {
    if (!userId || !organizationId) {
      console.warn('[AUDIT] Saltando log - Usuario u organización no definidos');
      return;
    }
    ...
    const ipAddress = req ? (req.ip || (req.connection as any)?.remoteAddress || (req.socket as any)?.remoteAddress) : null;
...
const auditMiddleware = (action: string, resourceType: string) => {
