...
const CapacitacionSingle: React.FC<CapacitacionSingleProps> = ({ capacitacionId, onBack }) => {
  ...
  const [evalForm, setEvalForm] = useState<{
    ...
    comentarios: string;
    fecha_evaluacion: string;
  }>({ empleado_id: '', tema_id: '', calificacion: '', comentarios: '', fecha_evaluacion: '' });
  ...
  const [editEvalInlineId, setEditEvalInlineId] = useState<string | null>(null);
  const [inlineEvalForm, setInlineEvalForm] = useState<{
    empleado_id: string;
    tema_id: string;
    calificacion: string;
    comentarios: string;
  ...
  const id = capacitacionId || (paramId ? parseInt(paramId) : null);
  ...
  const fetchCapacitacion = async (): Promise<void> => {
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Error al cargar la capacitación" });
      handleBack();
    } finally {
      setLoading(false);
    }
  ...
  const fetchAsistentes = async (): Promise<void> => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Error al cargar asistentes" });
    } finally {
  ...
  const fetchPersonal = async (): Promise<void> => {
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Error al cargar personal" });
    }
  };
  ...
  const fetchTemas = async (): Promise<void> => {
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Error al cargar temas" });
    }
  };
  ...
  const fetchEvaluaciones = async (): Promise<void> => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Error al cargar evaluaciones" });
    } finally {
  ...
  const handleSave = async (formData: CapacitacionFormData): Promise<void> => {
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Error al actualizar la capacitación" });
    }
  };
  ...
  const handleEliminarAsistente = async (asistente: Asistente): Promise<void> => {
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Error al eliminar asistente" });
    }
  };
  ...
  const getEstadoIcon = (estado?: string): React.ReactElement => {
    switch (estado?.toLowerCase()) {
      case 'programada':
  ...
  const buscarEmpleados = async (query: string): Promise<void> => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Error al buscar empleados" });
    } finally {
      setBuscando(false);
    }
  };
  ...
  const handleAgregarAsistente = async (empleado: Personal): Promise<void> => {
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Error al agregar asistente" });
    }
  };
  ...
  const handleStartInlineEval = (ev: Evaluacion | null = null): void => {
    if (ev) {
      ...
      setInlineEvalForm({
        empleado_id: ev.empleado_id.toString(),
        tema_id: ev.tema_id,
        calificacion: ev.calificacion.toString(),
  ...
  const handleInlineEvalChange = (field: string, value: string): void => {
  ...
  const handleSaveInlineEval = async (): Promise<void> => {
    try {
      if (editEvalInlineId === 'new') {
        await capacitacionesService.addEvaluacion(id!, {
          empleado_id: parseInt(inlineEvalForm.empleado_id),
          tema_id: inlineEvalForm.tema_id,
          calificacion: parseInt(inlineEvalForm.calificacion),
          comentarios: inlineEvalForm.comentarios,
          fecha_evaluacion: inlineEvalForm.fecha_evaluacion
        });
        toast({ title: "Éxito", description: "Evaluación agregada" });
      } else {
        await capacitacionesService.updateEvaluacion(id!, editEvalInlineId, {
          calificacion: parseInt(inlineEvalForm.calificacion),
          comentarios: inlineEvalForm.comentarios,
          fecha_evaluacion: inlineEvalForm.fecha_evaluacion
        });
        toast({ title: "Éxito", description: "Evaluación actualizada" });
      }
      setEditEvalInlineId(null);
      setInlineEvalForm({ empleado_id: '', tema_id: '', calificacion: '', comentarios: '', fecha_evaluacion: '' });
      fetchEvaluaciones();
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: "Error al guardar evaluación" });
    }
  };
  ...
  const handleDeleteEvaluacion = async (evalId: string): Promise<void> => {
    if (window.confirm('¿Eliminar esta evaluación?')) {
      try {
        await capacitacionesService.deleteEvaluacion(id!, evalId);
        toast({ title: "Éxito", description: "Evaluación eliminada" });
        fetchEvaluaciones();
      } catch (error) {
        toast({ variant: "destructive", title: "Error", description: "Error al eliminar evaluación" });
      }
    }
  };
