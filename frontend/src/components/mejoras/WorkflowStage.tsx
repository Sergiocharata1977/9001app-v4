...
import type { WorkflowStageProps } from '@/types/mejoras';
...
const WorkflowStage: React.FC<WorkflowStageProps> = ({ config, states, currentState }) => {
  if (!config) {
    return null; // No renderizar nada si no hay configuración para esta etapa
  }
  
  const { emoji, title, subtitle, bgColor, borderColor, titleColor }: StageConfig = config;
