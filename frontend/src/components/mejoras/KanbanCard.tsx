...
import type { KanbanCardProps, PrioridadVariantMap, InfoRowProps } from '@/types/mejoras';

const prioridadVariantMap: PrioridadVariantMap = {
  Alta: 'destructive',
  Media: 'warning',
  Baja: 'default',
};
...
export function KanbanCard({ hallazgo }: KanbanCardProps): React.JSX.Element {
      ...
      <Card className="bg-white dark:bg-gray-800 hover:shadow-xl transition-shadow border-l-4 border-transparent hover:border-blue-500">
        <CardHeader className="p-4">
          <div className="flex justify-between items-start">
            ...
            <Badge variant={prioridadVariantMap[hallazgo.prioridad] || 'secondary'}>
              {hallazgo.prioridad}
            </Badge>
          </div>
          <CardTitle className="text-base font-bold mt-1 text-gray-800 dark:text-white">
            {hallazgo.titulo}
          </CardTitle>
        </CardHeader>
        <CardContent className="p-4 pt-0 space-y-3">
          ...
          <div className="space-y-2 pt-2 border-t dark:border-gray-700">
            ...
            <InfoRow 
              icon={Calendar} 
              text={new Date(hallazgo.fechaDeteccion).toLocaleDateString()} 
            />
