...
import type { AccionItemProps, WorkflowAction } from '@/types/mejoras';
...
const getActionTypeInfo = (type: string): ActionTypeInfo => {
  switch (type) {
    case 'inmediata':
      return {
...
const AccionItem: React.FC<AccionItemProps> = ({ accion }) => {
  const { title, className } = getActionTypeInfo(accion.id);

  return (
    <div className={cn('flex items-start p-3 bg-white rounded-lg border-l-4 shadow-sm', className)}>
      <div className="flex-grow">
        <p className="font-semibold text-gray-800">{title}</p>
        <p className="text-sm text-gray-600 mt-1">{accion.description}</p>
