...
import type { Hallazgo, Mejora } from '@/types/mejoras';

interface CardProps {
  item: Hallazgo | Mejora;
  isOverlay?: boolean;
  onClick?: () => void;
}

export function Card({ item, isOverlay = false, onClick }: CardProps): React.JSX.Element {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  const isHallazgo = item.tipo === 'hallazgo';
  const itemNumber = isHallazgo ? (item as Hallazgo).numeroHallazgo : (item as Mejora).numeroMejora;
  const itemTitle = isHallazgo ? item.titulo : (item as Mejora).descripcion || 'Acción sin descripción';
